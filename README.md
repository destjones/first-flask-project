...
- 🎯 Entry points to execute your program using `python -m <first_flask_project>` or `$ first_flask_project` with basic CLI argument parsing.
...
---
# first_flask_project Flask Application

Awesome first_flask_project created by destjones
...
```bash
git clone https://github.com/destjones/first-flask-project first_flask_project
...
```bash
pip install first_flask_project
```
...
```bash
first_flask_project create-db   # run once
first_flask_project populate-db  # run once (optional)
first_flask_project add-user -u admin -p 1234  # ads a user
first_flask_project run
```
