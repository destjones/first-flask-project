...
original_author="destjones"
original_name="first_flask_project"
original_urlname="first-flask-project"
original_description="Awesome first_flask_project created by destjones" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/first_flask_project "${name}"
...
then
    ...
    echo "then run 'first_flask_project' or 'python -m first_flask_project'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'first_flask_project' or 'python -m first_flask_project' or 'poetry run first_flask_project'"
fi
